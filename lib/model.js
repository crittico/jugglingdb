...
AbstractClass.all = function all(params, cb) {
    ...
    this.schema.adapter.all(this.modelName, params, function (err, data) {
        if (params.returnCursor && data) cb(err, data.stream()); else {
        if (data && data.map) {
...
/*****************************
 * ADDITIONS - CUSTOM METHODS
 *****************************/
AbstractClass.aggregate = function (params, cb) {
    this.schema.adapter.aggregate(this.modelName, cb, params);
};

AbstractClass.mapReduce = function (map, reduce, options, cb) {
    this.schema.adapter.mapReduce(this.modelName, cb, map, reduce, options);
};

AbstractClass.stats = function (cb) {
    this.schema.adapter.stats(this.modelName, cb);
};

AbstractClass.exp = function (params, cb) {
    this.schema.adapter.exp(this.modelName, cb, params);
};
